	...
	protected function render() {
		...
		if ( $loop->have_posts() ) {
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				$output .= machic_wishlist_shortcode();

				$output .= machic_featured_icon();

				$output .= machic_compare_shortcode();

				if($quickview == '1'){
				$output .= '<a href="'.$product->get_id().'" class="detail-bnt quickview animated"><i class="klbth-icon-eye-empty"></i></a>';
				}
