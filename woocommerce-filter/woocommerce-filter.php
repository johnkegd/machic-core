...
function machic_remove_klb_filter(){
	...
	if(! empty( $_chosen_attributes ) || isset($_GET['filter_cat']) || 0 < $min_price || 0 < $max_price || machic_stock_status() == 'instock' || machic_on_sale() == 'onsale'){
		...
		$output .= '<li><a href="'.esc_url(remove_query_arg('stock_status')).'" class="remove-filter-element stock_status">'.esc_html__('In Stock','machic').'</a></li>';
		}
		
		if(machic_on_sale() == 'onsale'){
		$output .= '<li><a href="'.esc_url(remove_query_arg('on_sale')).'" class="remove-filter-element on_sale">'.esc_html__('On Sale','machic').'</a></li>';
		}
...
function machic_woocommerce_product_query_tax_query( $tax_query, $instance ) {
	...
	if(taxonomy_exists('location')){
		if(machic_location() != 'all'){
			$tax_query[] = array(
				'taxonomy' => 'location',
				'field' 	=> 'slug',
				'terms' 	=> machic_location(),
			);
		}
	}
...
if(get_theme_mod('machic_shop_single_orderonwhatsapp', 0) == 1 || machic_ft() == 'orderonwhatsapp'){
	...
	function machic_order_on_whatsapp(){
		...
		echo '<a href="https://wa.me/'.esc_attr($number).'?text='.get_the_title().'" class="button button-primary">'.esc_html__('Order on WhatsApp','machic-core').'</a>';
		echo '</div>';
	}
}
