...
if ( ! function_exists( 'machic_breadcrubms' ) ) {
    function machic_breadcrubms() {
        ...
        $separator  = '';
        $link       = apply_filters( 'machic_breadcrumb_link', '<li><a  href="%1$s" title="%2$s" rel="bookmark">%2$s</a> </li>  ' );
        $current    = apply_filters( 'machic_breadcrumb_current', '<li><span>%s</span></li>' );
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : esc_html__( 'Our Blog', 'machic' );
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'machic' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'machic' ) );
        }
        ...
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . esc_html__( 'Category : ', 'machic' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
                $out .= $separator . esc_html__( 'Tag : ', 'machic' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf( $current, esc_html__( 'Error 404', 'machic' ) );
            } else if ( is_search() ) {
                $out .= $separator . sprintf( $current, esc_html__( 'Search', 'machic' ) );
            }

        }

        $out .= '';
        return  '<ul class="breadcrumb-menu">'.apply_filters( 'machic_breadcrumbs_out', $out ).'</ul>';
    }
}
// Hooks into the machic theme
add_action( 'machic_before_template_header', 'machic_breadcrubms' );